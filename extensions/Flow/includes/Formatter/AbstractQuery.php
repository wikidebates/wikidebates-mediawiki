...
abstract class AbstractQuery {
	...
	protected function loadMetadataBatch( $results ) {
		...
		$rootPostIds = [];
		$uncachedRootLookups = [];
		foreach ( $postIds as $alpha => $postId ) {
			if ( isset( $this->rootPostIdCache[$alpha] ) ) {
				$rootPostIds[$alpha] = $this->rootPostIdCache[$alpha];
			} else {
				$uncachedRootLookups[$alpha] = $postId;
			}
		}
		if ( $uncachedRootLookups ) {
			$rootPostIds += array_filter( $this->treeRepository->findRoots( $uncachedRootLookups ) );
		}

		$rootPostRequests = [];
		$rootPostIdsToFetch = [];
		$identityMap = [];
		foreach ( $rootPostIds as $alpha => $postId ) {
			$rootAlpha = $postId->getAlphadecimal();
			if ( !isset( $this->identityMap[$rootAlpha] ) ) {
				$rootPostIdsToFetch[$rootAlpha] = $postId;
			}

			if ( !isset( $this->postCache[$rootAlpha] ) ) {
				$rootPostRequests[] = [ 'rev_type_id' => $postId ];
			}
		}

		// these tree identity maps are required for determining where a reply goes when
		// replying to a specific post. Only fetch the roots we do not already know.
		if ( $rootPostIdsToFetch ) {
			$identityMap = $this->treeRepository->fetchSubtreeIdentityMap(
				array_values( $rootPostIdsToFetch )
			);
		}
		...
		$rootPosts = [];
		foreach ( $rootPostIds as $rootPostId ) {
			$rootAlpha = $rootPostId->getAlphadecimal();
			if ( isset( $this->postCache[$rootAlpha] ) ) {
				$root = $this->postCache[$rootAlpha];
				$rootPosts[$rootAlpha] = $root;
		...
		$workflowRequests = [];
		$workflows = [];
		foreach ( array_merge( $rootPostIds, $workflowIds ) as $workflowId ) {
			$workflowAlpha = $workflowId->getAlphadecimal();
			if ( isset( $this->workflowCache[$workflowAlpha] ) ) {
				$workflows[$workflowAlpha] = $this->workflowCache[$workflowAlpha];
			} else {
				$workflowRequests[$workflowAlpha] = $workflowId;
			}
		}
		if ( $workflowRequests ) {
			$workflows += $this->storage->getMulti( 'Workflow', array_values( $workflowRequests ) ) ?: [];
		}
		...
		foreach ( $previousRevisionIds as $revisionType => $ids ) {
			...
			$uncachedIds = [];
			foreach ( $ids as $id ) {
				if ( !isset( $this->revisionCache[$id->getAlphadecimal()] ) ) {
					$uncachedIds[] = $id;
				}
			}
			if ( !$uncachedIds ) {
			...
			$found = $this->storage->getMulti( $revisionType, $uncachedIds );
		...
		foreach ( $collectionIds as $revisionType => $ids ) {
			$queries = [];
			foreach ( $ids as $uuid ) {
				$cacheKey = $revisionType . ':' . $uuid->getAlphadecimal();
