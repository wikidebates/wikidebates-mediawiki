...
( function () {
	...
	mw.flow.ui.EditorWidget.static.isVisualEditorSupported = function () {
		...
		var veInitModule = OO.ui.isMobile() ?
			'ext.visualEditor.mobileArticleTarget' :
			'ext.visualEditor.desktopArticleTarget.init';

		return !!(
			mw.loader.getState( veInitModule ) &&
			typeof VisualEditorSupportCheck !== 'undefined' &&
	...
	mw.flow.ui.EditorWidget.static.preload = function () {
		var conf, modules, veInitModule;
		if ( !this.preloadPromise ) {
			if ( this.isVisualEditorSupported() ) {
				conf = mw.config.get( 'wgVisualEditorConfig' );
				veInitModule = OO.ui.isMobile() ?
					'ext.visualEditor.mobileArticleTarget' :
					'ext.visualEditor.desktopArticleTarget.init';

				modules = [ 'ext.flow.visualEditor', veInitModule ].concat(
