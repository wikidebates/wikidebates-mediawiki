	...
	$( function () {
		...
		if ( initializer.setBoardDom( $board ) ) {
			...
			initializer.initDataModel( {
				pageTitle: pageTitle,
				tocPostsLimit: 50,
				renderedTopics: $component.find( '.flow-topic' ).length,
		...
		// Preload VisualEditor only when an editor is likely to be used, and do it
		// outside the critical path of the initial board render.
		if ( initializer.hasInteractiveEditorTargets() ) {
