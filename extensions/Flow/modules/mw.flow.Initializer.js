...
( function () {
	 ...
	 * @return {jQuery}
	 */
	mw.flow.Initializer.prototype.getBoardDom = function () {
		return this.$board || $( [] );
	};

	/**
	 * Get the current FlowBoard instance if available.
	 *
	 * @return {Object|null}
	 */
	mw.flow.Initializer.prototype.getFlowBoardInstance = function () {
		if ( this.flowBoard ) {
		...
		return this.flowBoard || null;
	};

	/**
	 * Whether this page is likely to need an interactive editor.
	 *
	 * @return {boolean}
	 */
	mw.flow.Initializer.prototype.hasInteractiveEditorTargets = function () {
		var $scope = this.$component || $( [] );

		return !!$scope.find(
			'.flow-edit-post-form, form.flow-newtopic-form, .flow-ui-edit-post-link, ' +
			'.flow-ui-summarize-topic-link, a.flow-ui-edit-title-link, a.flow-reply-link, ' +
	...
	mw.flow.Initializer.prototype.initializeWidgets = function () {
		...
		this.setupNavigationWidget( this.$component.find( '.flow-board-navigation' ).first() );

		// Set up new topic widget
		this.setupNewTopicWidget( this.$component.find( 'form.flow-newtopic-form' ).first() );

		// Set up description widget
		this.setupDescriptionWidget( this.$component.find( '.flow-ui-boardDescriptionWidget' ).first() );
		...
		this.system.connect( this, {
			...
			resetBoardEnd: function ( data ) {
				...
				setTimeout( function () {
					...
					self.$component.find( '.flow-board-navigation' ).first().append( self.navWidget.$element );

					self.setBoardDom( $( boardEl ) );

					self.replaceReplyForms( self.$board );

					self.setupNewTopicWidget( self.$component.find( 'form.flow-newtopic-form' ).first() );
	...
	mw.flow.Initializer.prototype.setupEditPostAction = function () {
		...
		this.$component.on( 'click', '.flow-ui-edit-post-link', function ( event ) {
			var editPostWidget,
				...
				flowBoard = self.getFlowBoardInstance();
	...
	mw.flow.Initializer.prototype.setupReplyLinkActions = function () {
		...
		this.$component.on( 'click', 'a.flow-reply-link', function () {
			...
			var replyWidget,
				...
				$targetContainer = self.$component.find( '#flow-post-' + replyTo + ' > .flow-replies, #flow-topic-' + replyTo ).first(),
				$existingWidget;

			if ( !$targetContainer.length ) {
				$targetContainer = $( '#flow-post-' + replyTo + ' > .flow-replies, #flow-topic-' + replyTo ).first();
			}
