...
function toggleCargoInputs() {
	if (jQuery('#use_cargo_toggle').attr('aria-checked') == 'true') {
		...
		jQuery('#cargo_table_input, label.cargo_field_type, div.pfFieldTypeDropdown, .allowed_values_input').removeClass('pf-hidden');
		jQuery('.is_hierarchy').parent().removeClass('pf-hidden');
	} else {
		jQuery('input[name="use_cargo"]').attr('value', '0');
		jQuery('#cargo_table_input, label.cargo_field_type, div.pfFieldTypeDropdown').addClass('pf-hidden');
		jQuery("input[name*='is_hierarchy_']").prop('checked', false);
		jQuery('.is_hierarchy').parent().addClass('pf-hidden');
		jQuery('.hierarchy_structure_input, div.hierarchy_structure').addClass('pf-hidden');
		jQuery('.allowed_values_input').removeClass('pf-hidden');
	}
}

jQuery.fn.createTemplateAddField = function( addAboveCurInstance ) {
	fieldNum++;
	const $newField = jQuery( '#starterField' ).clone().removeClass( 'pf-hidden' ).removeAttr( 'id' );
	...
	$newField.find( '.pfComboBox' ).each( function() {
		( new pf.ComboBoxInput() ).apply( $( this ) );
	} );
...
function toggleHierarchyInput(containerElement) {
	if (containerElement.find( "input[name*='is_hierarchy_']" ).prop('checked')) {
		containerElement.find( ".allowed_values_input" ).addClass('pf-hidden');
		containerElement.find( ".hierarchy_structure_input" ).removeClass('pf-hidden');
		containerElement.find( ".hierarchy_structure" ).removeClass('pf-hidden');
		...
		containerElement.find( ".hierarchy_structure_input" ).addClass('pf-hidden');
		containerElement.find( ".hierarchy_structure" ).addClass('pf-hidden');
		containerElement.find( ".allowed_values_input" ).removeClass('pf-hidden');
	}
}
...
function setHierarchyPlaceholder( textareaElement ) {
	if (textareaElement.val() === "") {
		...
		textareaElement.addClass( 'pf-placeholder-text' );
...
function removeHierarchyPlaceholder( textareaElement ) {
	textareaElement.val( '' );
	textareaElement.removeClass( 'pf-placeholder-text' );
...
$( () => {
	...
	jQuery( "div.pfTemplateNameBlankError" ).hide();
	jQuery( "div.pfCargoTableNameBlankError" ).hide();
	jQuery( "label.delimiter, div.delimiter, label.hierarchy_structure_input, div.hierarchy_structure" ).addClass( 'pf-hidden' );
	jQuery( "div.delimiter" ).addClass( 'pf-delimiter-field' );
	...
	jQuery( ".removeButton" ).click( function() {
		// Remove the encompassing div for this instance.
		jQuery( this ).closest( ".fieldBox" ).remove();
	} );
