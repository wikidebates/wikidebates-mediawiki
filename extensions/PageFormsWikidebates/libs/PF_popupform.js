...
window.ext.popupform = ( function() {
	...
	const padding = 20;
	let reload;
	function setVisible( $elem, isVisible, callback, duration ) {
		const transitionDuration = duration ?? 200;

		if ( brokenBrowser ) {
			$elem.toggle( isVisible );
			$elem.toggleClass( 'pf-popupform-is-hidden', !isVisible );
			if ( callback ) {
				callback();
			}
			return;
		...
		if ( isVisible ) {
			...
			requestAnimationFrame( () => {
				$elem.removeClass( 'pf-popupform-is-hidden' );
				if ( callback ) {
					setTimeout( callback, transitionDuration );
				}
			} );
		} else {
			$elem.addClass( 'pf-popupform-is-hidden' );
			setTimeout( () => {
				$elem.hide();
				if ( callback ) {
					callback();
	...
	function hideWithTransition( $elem, callback ) {
		setVisible( $elem, false, callback, $elem === $waitIndicator ? 200 : 160 );
	}
	...
	function handleCloseFrame( event ){
		...
		hideWithTransition( container, () => {
				hideWithTransition( $background, () => {
					$wrapper.remove();
				} );
		});
		return false;
	...
	function showWithTransition( $elem, callback ) {
		setVisible( $elem, true, callback, $elem === $waitIndicator ? 200 : 160 );
	}

	function setOpacityTransition( $elem, time, target, callback ) {
		if ( target > 0 ) {
			setVisible( $elem, true, callback, time );
		} else {
			setVisible( $elem, false, callback, time );
		}
	}
	...
	function showForm() {
		...
		$waitIndicator.addClass( 'pf-popupform-is-hidden' ).hide();
		container.addClass( 'pf-popupform-is-hidden' ).hide();
		...
		// reveal background and wait indicator
		if ( !brokenChrome ) {
			$background.show();
			requestAnimationFrame( () => {
				$background.removeClass( 'pf-popupform-is-hidden' );
			} );
		}
		showWithTransition( $waitIndicator );
...
//			left: "0",
		...
		if ( ! containerAlreadyVisible ) {
				hideWithTransition( $waitIndicator, () => {
				setOpacityTransition( container, 400, 1 );
			} );
		}
