...
function getDependentFieldSourcesByName() {
	const dependentSources = Object.create( null );
	const dependentPairs = mw.config.get( 'wgPageFormsDependentFields' ) || [];

	dependentPairs.forEach( ( pair ) => {
		if ( pair && pair[ 0 ] ) {
			dependentSources[ pair[ 0 ] ] = true;
		}
	} );
...
function getDependentSourceSelector() {
	const dependentSources = Object.keys( getDependentFieldSourcesByName() );
	if ( dependentSources.length === 0 ) {
		return '';
	}

	return dependentSources.map( ( fieldName ) => {
		const escaped = fieldName.replace( /(["\\])/g, '\\$1' );
		return '[name="' + escaped + '"], [origname="' + escaped + '"], [origName="' + escaped + '"]';
	} ).join( ', ' );
}

function getPFData() {
	const $pfForm = getPFForm();
	let pfData = $pfForm.data( 'PageForms' );

	if ( !pfData ) {
		setupPF();
		pfData = $pfForm.data( 'PageForms' );
	}
...
function createFormErrorHeader( message ) {
	const scriptPath = mw.config.get( 'wgPageFormsScriptPath' );
	const $icon = $( '<img>' ).attr( 'src', scriptPath + '/skins/MW-Icon-AlertMark.png' );
	const $header = $( '<div>' )
		.attr( 'id', 'form_error_header' )
		.addClass( 'errorbox pf-form-error-header cdx-message cdx-message--block cdx-message--error' )
		.append( $icon, ' ', message );
	const $clear = $( '<div>' ).addClass( 'pf-form-clear' );

	return $( '<div>' ).append( $header, $clear );
}

function ensureFormErrorHeader( message ) {
	var $target;
	...
	if ( $( '#contentSub' ).length ) {
		$target = $( '#contentSub' );
	} else if ( $( '.page-heading .tagline' ).length ) {
		$target = $( '.page-heading .tagline' );
	} else {
		$target = $( '#firstHeading, .firstHeading' ).first().parent();
	}

	$target.append( createFormErrorHeader( message ).contents() );
}

const pfModulePromises = {};
let pfIntersectionObserver = null;

function loadPFModule( moduleName ) {
	if ( !pfModulePromises[moduleName] ) {
		pfModulePromises[moduleName] = mw.loader.using( moduleName );
	}

	return pfModulePromises[moduleName];
}

function shouldLazyLoadImmediately( element ) {
	const rect = element.getBoundingClientRect();
	const viewportHeight = window.innerHeight || document.documentElement.clientHeight || 0;
	return rect.top < viewportHeight * 1.5 && rect.bottom > -viewportHeight * 0.25;
}

function createPFIntersectionObserver() {
	if ( pfIntersectionObserver !== null || typeof window.IntersectionObserver !== 'function' ) {
		return pfIntersectionObserver;
	}

	pfIntersectionObserver = new window.IntersectionObserver( ( entries, observer ) => {
		entries.forEach( ( entry ) => {
			...
			const initializer = $( entry.target ).data( 'pfLazyInit' );
...
function registerLazyWidget( $elements, initializer ) {
	const observer = createPFIntersectionObserver();

	$elements.each( function() {
		const $element = $( this );
		if ( $element.data( 'pfLazyRegistered' ) ) {
			return;
		}

		$element.data( 'pfLazyRegistered', true );
		$element.data( 'pfLazyInit', initializer );

		if ( shouldLazyLoadImmediately( this ) || observer === null ) {
			initializer();
			return;
		}
...
function initTokensIn( $root ) {
	const $tokens = $root.find( '.pfTokens' ).addBack( '.pfTokens' )
		.not( '.multipleTemplateStarter .pfTokens, .select2-container, .pf-tokens-initialized' );

	if ( $tokens.length === 0 ) {
		return;
	}

	mw.loader.using( [ 'ext.pageforms.select2', 'ext.pageforms.select2.styles' ] ).then( () => {
		$tokens.each( function() {
			const $element = $( this );
			if ( $element.hasClass( 'pf-tokens-initialized' ) || $element.next( '.select2' ).length ) {
				return;
			}
			const tokens = new pf.select2.tokens();
			tokens.apply( $element );
			$element.addClass( 'pf-tokens-initialized' );
		} );
	} );
}

function initAutoGrowIn( $root ) {
	const $elements = $root.find( '.autoGrow' ).addBack( '.autoGrow' ).not( '.pf-autogrow-initialized' );
	if ( $elements.length === 0 ) {
		return;
	}

	loadPFModule( 'ext.pageforms.autogrow' ).then( () => {
		$elements.filter( ':not(.pf-autogrow-initialized)' ).each( function() {
			$( this ).addClass( 'pf-autogrow-initialized' ).autoGrow();
		} );
	} );
}
...
function initRatingsIn( $root ) {
	...
	mw.loader.using( [ 'ext.pageforms.rating', 'ext.pageforms.rating.styles' ] ).then( () => {
		$elements.filter( ':not(.pf-rating-initialized)' ).each( function() {
			$( this ).addClass( 'pf-rating-initialized' ).applyRatingInput();
		} );
	} );
}

function initTreesIn( $root ) {
	const $elements = $root.find( '.pfTreeInput' ).addBack( '.pfTreeInput' ).not( '.pf-tree-initialized' );
	if ( $elements.length === 0 ) {
		return;
	}

	loadPFModule( 'ext.pageforms.jstree' ).then( () => {
		$elements.filter( ':not(.pf-tree-initialized)' ).each( function() {
			$( this ).addClass( 'pf-tree-initialized' ).applyJSTree();
		} );
	} );
}

function initImagePreviewIn( $root ) {
	if ( $root.find( '.pfImagePreview' ).addBack( '.pfImagePreview' ).length === 0 ) {
		return;
	}

	loadPFModule( 'ext.pageforms.imagepreview' );
}

function initCalendarsIn( $root ) {
	if ( $root.find( '.pfFullCalendarJS' ).addBack( '.pfFullCalendarJS' ).length === 0 ) {
		return;
	}

	loadPFModule( 'ext.pageforms.fullcalendar' );
}

function initVisibleWidgetsIn( $root ) {
	registerLazyWidget( $root.find( '.pfTokens' ).addBack( '.pfTokens' ), () => {
		initTokensIn( $root );
	} );
	registerLazyWidget( $root.find( '.autoGrow' ).addBack( '.autoGrow' ), () => {
		initAutoGrowIn( $root );
	} );
	registerLazyWidget( $root.find( '.pfRating' ).addBack( '.pfRating' ), () => {
		initRatingsIn( $root );
	} );
	registerLazyWidget( $root.find( '.pfTreeInput' ).addBack( '.pfTreeInput' ), () => {
		initTreesIn( $root );
	} );
	registerLazyWidget( $root.find( '.pfFullCalendarJS' ).addBack( '.pfFullCalendarJS' ), () => {
		initCalendarsIn( $root );
	} );
	initImagePreviewIn( $root );
}
...
$.fn.PageForms_registerInputValidation = function(valfunction, param) {
	...
	pfData.validationFunctions.push( validationData );

	if ( ! pfData.validationFunctionsByInput[inputId] ) {
		pfData.validationFunctionsByInput[inputId] = [];
	}

	pfData.validationFunctionsByInput[inputId].push( validationData );
...
$.fn.PageForms_registerInputInit = function( initFunction, param, noexecute ) {
	...
	// create entry
	if ( ! pfData.initFunctions[this.attr("id")] ) {
		pfData.initFunctions[this.attr("id")] = [];
	}

	// record initialization function
	pfData.initFunctions[this.attr("id")].push({
	...
	if ( this.attr("id") && pfdata ) {
		...
		for ( let i = 0; i < pfdata.validationFunctions.length; i++ ) {
			if ( typeof pfdata.validationFunctions[i] !== 'undefined' &&
				pfdata.validationFunctions[i].input === inputId ) {
				delete pfdata.validationFunctions[i];
			}
		}

		delete pfdata.validationFunctionsByInput[inputId];
	}
...
$.fn.PageForms_unregisterInputInit = function() {
	...
	if ( this.attr("id") ) {
		delete getPFData().initFunctions[this.attr("id")];
	}
...
function showDiv( div_id, $instanceWrapperDiv, initPage ) {
	...
	$elem.each( function() {
		const $current = $( this );
		if ( $current.css( 'display' ) === 'none' || initPage ) {
			$current.show().removeClass( 'pf-hidden-by-select' );
		}
	} );
...
function hideDiv( div_id, $instanceWrapperDiv, initPage ) {
	...
	$elem.each( function() {
		const $current = $( this );
		if ( $current.css( 'display' ) !== 'none' || !$current.hasClass( 'pf-hidden-by-select' ) ) {
			$current.addClass( 'pf-hidden-by-select' ).hide();
		}
	} );
...
$.fn.validateMandatoryField = function() {
	const $field = this.find( '.mandatoryField' );
	...
	if ( fieldVal === null ) {
		...
		isEmpty = ( normalizeValue( fieldVal ) === '' );
	}

	if ( isEmpty && $field.hasClass( 'visualeditor' ) ) {
		const target = $field.data( 'veForAllTarget' );
		let surfaceHtml = '';
		let $surfaceHtml;
		let surfaceText = '';
		let hasMeaningfulNode = false;

		if ( target && target.getSurface && target.getSurface() !== null ) {
			try {
				surfaceHtml = target.getSurface().getHtml() || '';
				$surfaceHtml = $( '<div>' ).html( surfaceHtml );
				surfaceText = normalizeValue( $surfaceHtml.text() );
				hasMeaningfulNode = $surfaceHtml.find( 'img,figure,table,ul,ol,li,video,audio,math,pre,blockquote' ).length > 0;
			} catch ( e ) {
				surfaceHtml = '';
				surfaceText = '';
			...
			if ( surfaceText !== '' || hasMeaningfulNode ) {
				if ( target.updateContent && $field.hasClass( 've-for-all-waiting-for-update' ) ) {
...
window.validateAll = function() {
	...
	$pfForm.find( ".errorMessage" ).remove();
	...
	$pfForm.find( ".multipleTemplateStarter" ).find( "span, div" ).addClass( "hiddenByPF" );

	$pfForm.find( ".multipleTemplateList" ).each( function() {
		if (! $(this).validateNumInstances() ) {
			num_errors += 1;
		}
	});

	$pfForm.find( "span.inputSpan.mandatoryFieldSpan" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateMandatoryField() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.comboboxSpan.mandatoryFieldSpan" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateMandatoryComboBox() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.dateInput.mandatoryFieldSpan" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateMandatoryDateField() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.radioButtonSpan.mandatoryFieldSpan" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateMandatoryRadioButton() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.checkboxesSpan.mandatoryFieldSpan" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateMandatoryCheckboxes() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "div.pfTreeInputWrapper.mandatory" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateMandatoryTree() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "div.pfPickerWrapper.mandatory" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).find('.pfPicker').validateMandatoryDatePicker() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.inputSpan.uniqueFieldSpan" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateUniqueField() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.inputSpan, span.comboboxSpan" ).not( ".hiddenByPF, .freeText, .pageSection" ).each( function() {
		if (! $(this).checkForPipes() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.URLInput" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateURLField() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.emailInput" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateEmailField() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.numberInput" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateNumberField() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.integerInput" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateIntegerField() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "span.dateInput" ).not( ".hiddenByPF" ).each( function() {
		if (! $(this).validateDateField() ) {
			num_errors += 1;
		}
	});
	$pfForm.find( "input.modifiedInput" ).not( ".hiddenByPF" ).each( function() {
		// No separate function needed.
		$(this).parent().addErrorMessage( 'pf_modified_input_error' );
		num_errors += 1;
	});

	const startDateInput = $pfForm.find( "span.startDateInput" ).not( ".hiddenByPF" )
	const endDateInput = $pfForm.find( "span.endDateInput" ).not( ".hiddenByPF" )

	if ( !validateStartEndDateField( startDateInput, endDateInput ) ) {
		num_errors += 1;
	}

	const startDateTimeInput = $pfForm.find( "span.startDateTimeInput" ).not( ".hiddenByPF" )
	const endDateTimeInput = $pfForm.find( "span.endDateTimeInput" ).not( ".hiddenByPF" )
	...
	if (num_errors > 0) {
		...
		ensureFormErrorHeader( mw.message( 'pf_formerrors_header' ).escaped() );
		...
		$pfForm.find( '.hiddenByPF' ).find( "input, select, textarea" ).not( ':disabled' )
		.prop('disabled', true)
...
$.fn.possiblyMinimizeAllOpenInstances = function() {
	...
	this.find('.multipleTemplateInstance').not('.minimized').each( function() {
		...
		const $display = $( '<div>' ).addClass( 'fieldValuesDisplay' );
		if ( valuesStr === '' ) {
			$display.append( $( '<em>' ).text( mw.message( 'pf-formedit-nodata' ).text() ) );
		} else {
			$display.html( valuesStr );
		}
		$instance.find( '.instanceMain' ).addClass( 'pf-hidden' );
		$instance.find( '.fieldValuesDisplay' ).remove();
		$instance.find( '.instanceRearranger' ).after( $display );
	});
};
...
$.fn.displayWizardScreen = function( screenNum, $wizardNav ) {
	...
	$wizardNav.append( '<div class="pf-wizard-clear"></div>' );
...
$.fn.addInstance = function( addAboveCurInstance ) {
	...
	const $starter = $wrapper.find(".multipleTemplateStarter").first();
	const $new_div = $starter
		.clone()
		.removeClass( 'multipleTemplateStarter pf-hidden' )
		.addClass( 'multipleTemplateInstance multipleTemplate pf-added-instance' ) // backwards compatibility
		.removeAttr( 'id' );
	...
	const $newInputs = $new_div.find("input, select, textarea");
	const bracketNum = '[' + num_elements + 'b]';
	const inputIdPrefix = 'input_' + num_elements + '_';
	...
	$newInputs.each(
		function() {
			...
			if ( this.name ) {
				...
				this.setAttribute( 'origName', old_name );
				this.name = this.name.replace(/\[num\]/g, bracketNum);
			}

			// Do the same thing with "feeds to map", which also
			// needs to be modified for each instance.
			const feedsToMap = this.getAttribute( 'data-feeds-to-map' );
			if ( feedsToMap !== null ) {
				this.setAttribute( 'data-feeds-to-map', feedsToMap.replace(/\[num\]/g, bracketNum ) );
			}
			...
			if (this.id) {
				...
				this.id = this.id.replace(/input_/g, inputIdPrefix);
				...
				if ( pfdata ) { // found data object?
					...
					// for the new input.
					const oldValidationData = pfdata.validationFunctionsByInput[old_id];
					if ( oldValidationData ) {
						for ( i = 0; i < oldValidationData.length; i++ ) {
							$(this).PageForms_registerInputValidation(
								oldValidationData[i].valfunction,
								oldValidationData[i].parameters
							);
						}
					}
				}
	...
	if ( addAboveCurInstance ) {
		...
		$multipleTemplateList.append( $new_div );
	}
...
$.fn.setDependentAutocompletion = function( dependentField, baseField, baseValue ) {
	...
	const previousRequest = $thisInput.data( 'pfDependentAutocompleteRequest' );
	...
	const request = $.ajax( {
		...
		$thisInput.removeData( 'pfDependentAutocompleteRequest' );
	} );

	$thisInput.data( 'pfDependentAutocompleteRequest', request );
	return request;
};
...
$.fn.setAutocompleteForDependentField = function( partOfMultiple ) {
	...
	const name = partOfMultiple ? ( $( this ).attr( 'origname' ) || $( this ).attr( 'origName' ) ) : $( this ).attr( 'name' );
	...
	$.each( dependent_on_me, function() {
		let $element, combobox, tokens,
			dependentField = this;

		if ( partOfMultiple ) {
			$element = $( self ).closest( '.multipleTemplateInstance' )
				.find( '[origname="' + dependentField + '"], [origName="' + dependentField + '"]' );
		} else {
		...
		if ( $element.hasClass( 'pfTokens' ) ) {
			...
			$element.setDependentAutocompletion( dependentField, name, curValue ).done( function() {
				tokens.refresh( $element );
			} );
		} else if ( $element.hasClass( 'pfComboBox' ) ) {
			combobox = new pf.ComboBoxInput( {} );
			$element.setDependentAutocompletion( dependentField, name, curValue ).done( function() {
				combobox.refresh( $element );
			} );
		} else {
...
$.fn.initializeJSElements = function( partOfMultiple ) {
	...
	$root.find(".pfShowIfSelected").each( function() {
	...
	$root.find(".pfShowIfChecked").each( function() {
		// Avoid duplicate calls on any one element.
		if ( !partOfMultiple && $(this).parents('.multipleTemplateWrapper').length > 0 ) {
			return;
		}
	...
	$root.find(".pfShowIfCheckedCheckbox").each( function() {
		// Avoid duplicate calls on any one element.
		if ( !partOfMultiple && $(this).parents('.multipleTemplateWrapper').length > 0 ) {
			return;
		}
	...
	if ( partOfMultiple ) {
		...
		$root.find( '.removeButton' ).off( 'click.pfRemoveInstance' ).on( 'click.pfRemoveInstance', function() {
			var $instance = $( this ).closest( '.multipleTemplateInstance' );

			if ( !$instance.length || $instance.hasClass( 'pf-removing-instance' ) ) {
				return false;
			}

			$instance.addClass( 'pf-removing-instance' );

			window.setTimeout( function() {
				$instance.find( 'input, select, textarea' ).each( function() {
		...
		// ...and the new adder
		$root.find('.addAboveButton').off( 'click.pfAddAbove' ).on( 'click.pfAddAbove', function() {
			$(this).addInstance( true );
			return false; // needed to disable <a> behavior
		});
	}

	$root.find('.pfComboBox').not('.multipleTemplateStarter .pfComboBox, .pf-combobox-initialized').each( function() {
		const $source = $( this );
		const inputType = new pf.ComboBoxInput( {} );
		inputType.apply( $source );
	});

	// Set the end date input to the value selected in start date
	$root.find("span.startDateInput").not(".hiddenByPF").find("input").last().off( 'blur.pfCopyStartDate' ).on( 'blur.pfCopyStartDate', function() {
		const endInput = $root.find("span.endDateInput").not(".hiddenByPF");
		...
		if ( endYearInput.val() === '' && endMonthInput.val() === '' && endDayInput.val() === '' ) {
			const $startWrapper = $( this ).closest( 'span.startDateInput' );
			const startYearVal = $startWrapper.find( ".yearInput" ).val();
			const startMonthVal = $startWrapper.find( ".monthInput" ).val();
			const startDayVal = $startWrapper.find( ".dayInput" ).val();
	...
	if ( partOfMultiple ) {
		initVisibleWidgetsIn( $root );
		if ( typeof $.fn.applyDatePicker === 'function' ) {
			$root.find('.pfDatePicker').applyDatePicker();
		}
		if ( typeof $.fn.applyDateTimePicker === 'function' ) {
			$root.find('.pfDateTimePicker').applyDateTimePicker();
		}
		if ( window.ext && ext.popupform && typeof ext.popupform.handlePopupFormLink === 'function' ) {
			$root.find('a.popupformlink').off( 'click.pfPopupForm' ).on( 'click.pfPopupForm', function(evt){
		...
		if ( typeof this.initializeSimpleUpload === 'function' ) {
			$root.find(".simpleUploadInterface").each( function() {
		...
		initVisibleWidgetsIn( $root );
		if ( typeof $.fn.applyDatePicker === 'function' ) {
			$root.find('.pfDatePicker').not(".multipleTemplateWrapper .pfDatePicker").applyDatePicker();
		}
		if ( typeof $.fn.applyDateTimePicker === 'function' ) {
			$root.find('.pfDateTimePicker').not(".multipleTemplateWrapper .pfDateTimePicker").applyDateTimePicker();
		}
		// Only defined if $wgPageFormsSimpleUpload == true.
		if ( typeof this.initializeSimpleUpload === 'function' ) {
			$root.find(".simpleUploadInterface").not(".multipleTemplateWrapper .simpleUploadInterface").each( function() {
				$(this).initializeSimpleUpload();
			});
		}
	}

	const dependentSourceSelector = getDependentSourceSelector();
	if ( dependentSourceSelector !== '' ) {
		const $dependentSources = $root.find( dependentSourceSelector ).addBack( dependentSourceSelector );
		$dependentSources.each( function() {
			$( this ).setAutocompleteForDependentField( partOfMultiple );
		} );
		$dependentSources.off( '.pfDependentAutocomplete' )
			.on( 'change.pfDependentAutocomplete blur.pfDependentAutocomplete', function() {
				$( this ).setAutocompleteForDependentField( partOfMultiple );
			} );
	}

	$root.find('.new-uuid').each( function() {
		$(this).val(window.pfGenerateUUID());
	});

	$root.find('[data-tooltip]').not('.multipleTemplateStarter [data-tooltip]').each( function() {
		const $tooltipText = $( '<p>' )
			.addClass( 'pf-tooltip-content' )
...
$( () => {
	...
	$pfForm.on( 'focusin', '.pfTokens, .autoGrow, .pfRating, .pfTreeInput', function() {
		const $target = $( this );
		if ( $target.hasClass( 'pfTokens' ) ) {
			initTokensIn( $target.closest( '.multipleTemplateInstance, #pfForm' ) );
		} else if ( $target.hasClass( 'autoGrow' ) ) {
			initAutoGrowIn( $target.closest( '.multipleTemplateInstance, #pfForm' ) );
		} else if ( $target.hasClass( 'pfRating' ) ) {
			initRatingsIn( $target.closest( '.multipleTemplateInstance, #pfForm' ) );
		} else if ( $target.hasClass( 'pfTreeInput' ) ) {
			initTreesIn( $target.closest( '.multipleTemplateInstance, #pfForm' ) );
		}
	} );
	...
	setTimeout( () => {
		...
		$( '#loadingMask' ).addClass( 'pf-loading-mask--active' );
		...
		try {
			...
			$pfForm.off( 'click.pfAddInstance', '.multipleTemplateAdder' ).on( 'click.pfAddInstance', '.multipleTemplateAdder', function() {
				$( this ).addInstance( false );
			} );
			const wgPageFormsHeightForMinimizingInstances = mw.config.get( 'wgPageFormsHeightForMinimizingInstances' );
			minimizeInstances( wgPageFormsHeightForMinimizingInstances );

			if ( $( '.multipleTemplateList' ).length ) {
				loadPFModule( 'ext.pageforms.sortable' ).then( () => {
					const SortableModule = require( 'ext.pageforms.sortable' );

					if ( !SortableModule || typeof SortableModule.create !== 'function' ) {
						return;
					}

					$( '.multipleTemplateList' ).each( function() {
						const $list = $( this );
						if ( $list.data( 'pfSortableInitialized' ) ) {
							return;
						}

						SortableModule.create( $list[0], {
							draggable: '.multipleTemplateInstance',
							handle: '.instanceRearranger',
							onStart: function() {
								$list.possiblyMinimizeAllOpenInstances();
							},
							onEnd: function() {
								$list.removeClass( 'currentFocus' );
								$list.find( '.multipleTemplateInstance' ).each( function( index ) {
									$( this ).attr( 'data-instance-order', index );
								} );
							}
						} );
						$list.data( 'pfSortableInitialized', true );
					} );
				} );
			}

			if ( mw.user.options.get( 'useeditwarning' ) ) {
				loadPFModule( 'ext.pageforms.editwarning' );
			}
			...
			$pfForm.off( 'submit.pfValidate' ).on( 'submit.pfValidate', () => validateAll() );

			} finally {
				$( '#loadingMask' ).removeClass( 'pf-loading-mask--active' );
				$( 'span.loadingImage' ).hide();
			}
...
$('form#pfForm').click( (e) => {
	...
	if ( $instance.hasClass('minimized') ) {
		...
		$instance.find( '.fieldValuesDisplay' ).remove();
		$instance.find( '.instanceMain' ).removeClass( 'pf-hidden' );
...
$('#pf-expand-all a').click(( event ) => {
	...
	$('.minimized').each( function() {
		...
		$( this ).find( '.instanceMain' ).removeClass( 'pf-hidden' );
		// Remove unhelpful styling added by VisualEditor.
