...
window.PF_TP_init = function( inputID, params ) { // minTime, maxTime, interval, format
	...
	.mouseover((evt) => {
		...
		.data( 'timeout', setTimeout(
			() => {
				jQuery( evt.currentTarget ).children().stop( true, true ).show();
			}, 400 ) );
	...
	.mouseout((evt) => {
		...
		.data( 'timeout', setTimeout(
			() => {
				jQuery( evt.currentTarget ).children().stop( true, true ).hide();
			}, 400 ) );
	...
	$inputShow
	.focus(() => {
		jQuery( '#' + inputID + '_tree>ul' ).stop( true, true ).show();
	});

	// hide timepicker when input loses focus
	$inputShow
	.blur(() => {
		jQuery( '#' + inputID + '_tree ul' ).stop( true, true ).hide();
	});
