...
class PFUtils {
	...
	public static function printRedirectForm(
		...
		$text = <<<END
	<p class="pf-redirect-spinner"><img src="$wgPageFormsScriptPath/skins/loading.gif" /></p>
...
END;
	 ...
	 */
	private static function detectFormRLRequirements( $html = '' ) {
		$requirements = [
			'needsSubmit' => false,
			'needsCheckboxes' => false,
			'needsPopup' => false,
			'needsCollapsible' => false,
			'needsSimpleUpload' => false,
			'needsEditWarning' => false,
			'needsOOUIAutocomplete' => false,
			'needsOOUIComboBox' => false
		];
		...
		$requirements['needsSubmit'] = strpos( $html, 'pf-save_and_continue' ) !== false;
		$requirements['needsCheckboxes'] = strpos( $html, 'checkboxesSpan select-all' ) !== false;
		$requirements['needsPopup'] =
			strpos( $html, 'popupformlink' ) !== false ||
			strpos( $html, 'popupforminput' ) !== false;
		$requirements['needsCollapsible'] = strpos( $html, 'mw-collapsible' ) !== false;
		$requirements['needsSimpleUpload'] = strpos( $html, 'simpleUploadInterface' ) !== false;
		$requirements['needsEditWarning'] = strpos( $html, 'pfSendBack' ) !== false || strpos( $html, 'pf_free_text' ) !== false;
		$requirements['needsOOUIAutocomplete'] = strpos( $html, 'pfAutocompleteInput' ) !== false;
		$requirements['needsOOUIComboBox'] = strpos( $html, 'pfComboBox' ) !== false;

		return $requirements;
	}

	public static function addFormRLModules( $parser = null, array $options = [] ) {
		...
		$requirements = self::detectFormRLRequirements( $options['formHtml'] ?? '' );
		...
		if ( $requirements['needsSubmit'] ) {
			$mainModules[] = 'ext.pageforms.submit';
			$mainModuleStyles[] = 'ext.pageforms.submit.styles';
		}

		if ( $requirements['needsCheckboxes'] ) {
			$mainModules[] = 'ext.pageforms.checkboxes';
			$mainModuleStyles[] = 'ext.pageforms.checkboxes.styles';
		}

		if ( $requirements['needsPopup'] ) {
			$mainModules[] = 'ext.pageforms.popupformedit';
		}

		if ( $requirements['needsEditWarning'] ) {
			$mainModules[] = 'ext.pageforms.editwarning';
		}

		if ( $requirements['needsOOUIAutocomplete'] ) {
			$mainModules[] = 'ext.pageforms.ooui.autocomplete';
		}

		if ( $requirements['needsOOUIComboBox'] ) {
			$mainModules[] = 'ext.pageforms.ooui.combobox';
		}

		if ( $requirements['needsCollapsible'] ) {
			$mainModules[] = 'jquery.makeCollapsible';
		}

		if ( $wgPageFormsSimpleUpload && $requirements['needsSimpleUpload'] ) {
			$mainModules[] = 'ext.pageforms.simpleupload';
		}

		$output->addModules( array_values( array_unique( $mainModules ) ) );
		$output->addModuleStyles( array_values( array_unique( $mainModuleStyles ) ) );
