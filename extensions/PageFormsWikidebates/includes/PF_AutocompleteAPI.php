...
class PFAutocompleteAPI extends ApiBase {
	...
	public function execute() {
		...
		if ( $baseprop !== null ) {
			...
			$data = $this->getAllValuesForCategory( $category, $substr, 3 );
	...
	private function getAllValuesForCategory( $category, $substring, $numLevels = 3 ) {
		global $wgPageFormsCacheAutocompleteValues, $wgPageFormsAutocompleteCacheTimeout;

		if ( !$wgPageFormsCacheAutocompleteValues ) {
			return PFValuesUtils::getAllPagesForCategory( $category, $numLevels, $substring );
		}

		$cache = PFFormUtils::getFormCache();
		$cacheKey = $cache->makeKey(
			'pf-autocomplete-category',
			md5( $category . '|' . $numLevels . '|' . rtrim( (string)$substring ) )
		);

		return $cache->getWithSetCallback(
			$cacheKey,
			$wgPageFormsAutocompleteCacheTimeout,
			function () use ( $category, $numLevels, $substring ) {
				return PFValuesUtils::getAllPagesForCategory( $category, $numLevels, $substring );
			}
		);
	}
	...
	private function getAllValuesForProperty(
		...
		$cacheKey = $cache->makeKey( 'pf-autocomplete-v4', md5( $cacheKeyString ) );
		return $cache->getWithSetCallback(
			$cacheKey,
			$wgPageFormsAutocompleteCacheTimeout,
	...
	private function getFastPropertyPrefixCondition( $db, $column, $substring, $replaceSpaces = false ) {
		...
		if ( $db->getType() === 'mysql' ) {
			return $column . $db->buildLike( $substring, $db->anyString() );
		}

		return PFValuesUtils::getSQLConditionForAutocompleteAtStartOfColumn(
			$column,
			$substring,
			false
		);
	...
	private function computeAllValuesForProperty(
		...
		if ( $substring !== null ) {
			$prefixCondition = $this->getFastPropertyPrefixCondition(
				$db,
				$valueField,
				$substring,
				$propertyHasTypePage
		...
		$res = $db->select( $tables, [ 'value' => "DISTINCT $valueField" ],
			$conditions, __METHOD__, $sqlOptions, $joinConds );

		$values = [];
		while ( $row = $res->fetchRow() ) {
			$values[] = str_replace( '_', ' ', $row['value'] );
		}
		$res->free();
		return $values;
	}
