...
class PFFormPrinter {
	...
	function multipleTemplateInstanceTableHTML( $form_is_disabled, $mainText ) {
		...
		$mainText = preg_replace( '~^(?:<p>\s*</p>\s*)+|(?:\s*<p>\s*</p>)+$~i', '', $mainText );
		$mainText = preg_replace( "~^(?:<p>\\s*)+(<div\\b[^>]*class=(?:\"|')pf-formlayout(?:[^\"']*)(?:\"|')[^>]*>)~i", '$1', $mainText );
		$mainText = preg_replace( '~(</div>)\s*(?:</p>\s*)+$~i', '$1', $mainText );
		...
		$text = '<div class="multipleTemplateInstanceTable">' .
			'<div class="instanceRearranger"></div>' .
			'<div class="instanceMain">' . $mainText . '</div>' .
			'<div class="instanceAddAbove">' . $addAboveButton . '</div>' .
			'<div class="instanceRemove">' . $removeButton . '</div>' .
			'</div>';
	...
	function multipleTemplateInstanceHTML( $template_in_form, $form_is_disabled, &$section ) {
		...
		$section = $this->convertLegacyFormTableHtml( $section );
	...
	function multipleTemplateEndHTML( $template_in_form, $form_is_disabled, $section ) {
		...
		$text = "\t\t" . Html::rawElement( 'div',
			[
				'class' => "multipleTemplateStarter pf-hidden",
			],
			$this->multipleTemplateInstanceTableHTML( $form_is_disabled, $section )
		) . "\n";
...
END;
	...
	private function normalizeFieldLayoutClass( $value ) {
	...
	private function buildFieldLayoutClasses( $formField ) {
		$classes = [ 'pf-formlayout__row' ];
		$classes[] = 'pf-field--' . $this->normalizeFieldLayoutClass( $formField->getInputType() ?: 'text' );
		$classes[] = 'pf-field-name--' . $this->normalizeFieldLayoutClass( $formField->getTemplateField()->getFieldName() );
		if ( $formField->isMandatory() ) {
			$classes[] = 'pf-formlayout__row--mandatory';
		}
		if ( $formField->isHidden() ) {
			$classes[] = 'pf-formlayout__row--hidden';
		}
		if ( $formField->isDisabled() ) {
			$classes[] = 'pf-formlayout__row--disabled';
		}
		if ( $formField->hasFieldArg( 'class' ) ) {
			$classes[] = trim( $formField->getFieldArg( 'class' ) );
		}
		return implode( ' ', array_filter( $classes ) );
	}

	private function buildFieldInputDataAttrs( $formField ) {
		$attrs = [
			'data-field-name' => $formField->getTemplateField()->getFieldName(),
			'data-input-type' => $formField->getInputType() ?: 'text'
		];
		if ( $formField->hasFieldArg( 'origName' ) ) {
			$attrs['data-orig-name'] = $formField->getFieldArg( 'origName' );
		}
		return $attrs;
	}


	private function stripStyleAttributeFromTagAttributes( $attrs ) {
		return preg_replace( '/\s+style=("[^"]*"|\'[^\']*\')/i', '', (string)$attrs );
	}

	private function convertLegacyFormTableHtml( $html ) {
		if ( !is_string( $html ) || strpos( $html, 'formtable' ) === false ) {
			return $html;
		}

		$html = preg_replace_callback(
			'~<table\b([^>]*)class=("|\')([^"\']*\bformtable\b[^"\']*)(?:\2)([^>]*)>~i',
			function ( $matches ) {
				$attrs = $this->stripStyleAttributeFromTagAttributes( $matches[1] . ' ' . $matches[4] );
				return '<div class="pf-formlayout pf-formlayout--legacy-converted" role="group"' . $attrs . '>';
			},
			$html
		);
		$html = preg_replace( '~</table>~i', '</div>', $html );
		$html = preg_replace( '~<tbody\b[^>]*>~i', '', $html );
		$html = preg_replace( '~</tbody>~i', '', $html );
		$html = preg_replace_callback(
			'~<tr\b([^>]*)>~i',
			function ( $matches ) {
				$attrs = $this->stripStyleAttributeFromTagAttributes( $matches[1] );
				return '<div class="pf-formlayout__row"' . $attrs . '>';
			},
			$html
		);
		...
		$html = preg_replace_callback(
			...
			function ( $matches ) {
				$attrs = $this->stripStyleAttributeFromTagAttributes( $matches[1] );
				return '<div class="pf-formlayout__label"' . $attrs . '>';
			},
			$html
		);
		...
		$html = preg_replace_callback(
			...
			function ( $matches ) {
				$attrs = $this->stripStyleAttributeFromTagAttributes( $matches[1] );
				return '<div class="pf-formlayout__input"' . $attrs . '>';
			},
			$html
		);
	...
	private function renderFormLayoutRow( $formField, $labelHtml, $inputHtml, array $extraAttrs = [] ) {
		$rowAttrs = array_merge( [
			'class' => $this->buildFieldLayoutClasses( $formField )
		], $extraAttrs );
		$labelCell = Html::rawElement( 'div', [ 'class' => 'pf-formlayout__label' ], $labelHtml );
		$inputCellAttrs = array_merge( [ 'class' => 'pf-formlayout__input' ], $this->buildFieldInputDataAttrs( $formField ) );
		$inputCell = Html::rawElement( 'div', $inputCellAttrs, $inputHtml );
		return Html::rawElement( 'div', $rowAttrs, $labelCell . $inputCell ) . "\n";
	}
	...
	function tableHTML( $tif, $instanceNum ) {
		...
		foreach ( $tif->getFields() as $formField ) {
			...
			$labelAttrs = [ 'class' => 'pf-formlayout__label-text' ];
			if ( $formField->isMandatory() ) {
				$labelAttrs['class'] .= ' pf-formlayout__label-text--mandatory';
			}
			$labelMarkup = Html::element( 'label',
				array_merge( [ 'for' => "input_$wgPageFormsFieldNum" ], $labelAttrs ),
				$labelText );
			if ( $formField->hasFieldArg( 'tooltip' ) ) {
				$labelMarkup .= Html::rawElement(
					'span',
					[
						'class' => 'pf-formlayout__tooltip',
						'title' => $formField->getFieldArg( 'tooltip' ),
						'aria-label' => $formField->getFieldArg( 'tooltip' )
					],
					'&#9432;'
				);
			}
			...
			$html .= $this->renderFormLayoutRow( $formField, $labelMarkup, $inputHTML, $labelCellAttrs );
		}

		$html = Html::rawElement( 'div', [ 'class' => 'pf-formlayout', 'role' => 'group' ], $html );
	...
	static function displayLoadingImage() {
		...
		$text .= Html::rawElement( 'div', [ 'class' => 'pf-loading-image__background' ], $loadingBGImage );
		$loadingImage = Html::element( 'img', [ 'src' => "$wgPageFormsScriptPath/skins/loading.gif" ] );
		$text .= Html::rawElement( 'div', [ 'class' => 'pf-loading-image__spinner' ], $loadingImage );
...
END;
				...
				// =====================================================
					...
					$section = substr_replace( $section, '<span class="pf-hidden-placeholder"></span>', $brackets_loc, $brackets_end_loc + 3 - $brackets_loc );
				// =====================================================
			...
			if ( $tif && $tif->allowsMultiple() ) {
				if ( $tif->getDisplay() == 'spreadsheet' ) {
					...
					if ( $tif->allInstancesPrinted() ) {
						...
						$loadingImage = Html::element( 'img', [ 'src' => "$wgPageFormsScriptPath/skins/loading.gif" ] );
						$text = Html::rawElement( 'div', [ 'id' => 'fullCalendarLoading1', 'class' => 'pf-hidden' ], $loadingImage );
				...
				$form_text .= $this->convertLegacyFormTableHtml( $this->tableHTML( $tif, 0 ) );
