...
class PFTextInput extends PFFormInput {
	...
	public static function uploadableHTML( $input_id, $delimiter = null, $default_filename = null, $cur_value = '', array $other_args = [] ) {
		...
		if ( $wgPageFormsSimpleUpload ) {
			$text = "\n" . '<img class="loading pf-hidden" src="' . $wgPageFormsScriptPath . '/skins/loading.gif"/>' . "\n";
	...
	public static function getHTML( $cur_value, $input_name, $is_mandatory, $is_disabled, array $other_args ) {
		...
		$inputAttrs = [
			...
			'class' => trim( $className . ' autoGrow pf-singleline-text' ),
			'rows' => 1,
			'cols' => $size
		];
		if ( $is_disabled ) {
		...
		$text = Html::element( 'textarea', $inputAttrs + [ 'name' => $input_name ], $cur_value );
