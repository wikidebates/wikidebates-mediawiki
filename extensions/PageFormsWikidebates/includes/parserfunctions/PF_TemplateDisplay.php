...
class PFTemplateDisplay {
	...
	private static function ratingText( $value ) {
		...
		$text = Html::rawElement( 'span', [ 'class' => 'pf-rating-display', 'style' => "--pf-rating-image:url('{$url}');" ],
			Html::element( 'span', [ 'class' => 'pf-rating-display__fill', 'style' => "--pf-rating-width:{$rate}%;" ] )
		);
		return $text;
	}
